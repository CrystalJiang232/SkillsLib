...
| `references/wuling/` | 五灵 hidden chamber (index, patterns, checklist, flagship) | Trigger: `五灵应象决` or HFT context | Skeleton — designated growth area |
...
**Status**: Framework fixed; index in place.
- **Quick checklist** → `references/checklist-hft.md` (isolation, lock-free, latency, kernel dimensions)
- **Pending**: detailed per-spirit patterns (`references/wuling/patterns.md`) and flagship examples (`references/wuling/flagship.md`) — designated growth area for the learning-from-codebase pass
